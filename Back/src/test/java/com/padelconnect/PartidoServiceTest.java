...
class PartidoServiceTest {
    ...
    // Fecha siempre en el futuro relativa al momento en que corren los tests
    // (evita que los tests se rompan con el paso del tiempo por usar una fecha fija).
    private static final String FECHA_FUTURA = LocalDate.now().plusDays(10).toString();
    ...
    private PartidoRequestDTO.PartidoRequestDTOBuilder partidoBase() {
        return PartidoRequestDTO.builder()
                .fecha(FECHA_FUTURA).hora("18:00").cancha("Club Padel").direccion("Calle 123").zona("CABA")
                .latitud(-34.6037).longitud(-58.3816).nivel("5ta")
                .precioPersona("$4.000").tipoCancha("Sintetico");
    }
    ...
    void testCrearPartidoConJugadoresFaltantes() {
        ...
        PartidoRequestDTO req = partidoBase().jugadoresFaltantes(2).build();
    ...
    void testAceptarInscripcion() {
        PartidoRequestDTO req = partidoBase().jugadoresFaltantes(1).build();
        PartidoResponseDTO partido = partidoService.crearPartido(req, org);
    ...
    void testNoSePuedeEliminarPartidoAMenosDeUnaHora() {
        ...
        PartidoRequestDTO req = partidoBase()
                .fecha(fechaHoy).hora(horaEnMediaHora).jugadoresFaltantes(1).build();
        PartidoResponseDTO partido = partidoService.crearPartido(req, org);
    ...
    void testListarPartidosExcluyeLlenos() {
        ...
        PartidoRequestDTO req = partidoBase().cancha("Club Padel Lleno").jugadoresFaltantes(1).build();
    ...
    @Test
    void testListarPartidosExcluyeLosQueYaPasaron() {
        // Partido de hace media hora: ya pasó, no debería listarse.
        String fechaHoy = LocalDate.now().toString();
        String horaHaceMediaHora = LocalTime.now().minusMinutes(30).truncatedTo(ChronoUnit.MINUTES).toString();
        PartidoRequestDTO reqPasado = partidoBase()
                .cancha("Club Pasado").fecha(fechaHoy).hora(horaHaceMediaHora).jugadoresFaltantes(1).build();
        PartidoResponseDTO partidoPasado = partidoService.crearPartido(reqPasado, org);

        // Partido futuro: sí debería listarse.
        PartidoRequestDTO reqFuturo = partidoBase().cancha("Club Futuro").jugadoresFaltantes(1).build();
        PartidoResponseDTO partidoFuturo = partidoService.crearPartido(reqFuturo, org);

        List<PartidoResponseDTO> listado = partidoService.listarPartidos(null, null, null, null, null, null, null, org);

        assertTrue(listado.stream().noneMatch(p -> p.getId().equals(partidoPasado.getId())));
        assertTrue(listado.stream().anyMatch(p -> p.getId().equals(partidoFuturo.getId())));
    }
}
