...
## Séptima ronda: mensaje al organizador, eliminar partido y ocultar partidos llenos

- **Mensaje al solicitar unirse**: `Inscripcion` ahora tiene un campo `mensaje` (opcional, hasta 300 caracteres). Al tocar "Sumarme al partido" se abre un modal para escribirle algo al organizador antes de enviar la solicitud; el organizador lo ve en la tarjeta de cada solicitud pendiente.
- **Eliminar un partido propio**: nuevo endpoint `DELETE /api/partidos/{id}`, solo para el organizador, que borra el partido y sus inscripciones asociadas (no había relación en cascada configurada, así que el service borra las inscripciones explícitamente antes del partido para no romper la foreign key). No se puede eliminar un partido a menos de 1 hora de su horario de inicio — la regla se aplica en el backend (autoritativa) y también en el frontend (el botón se reemplaza por un aviso cuando falta menos de una hora).
- **Partidos llenos ya no aparecen en "Explorar Partidos"**: `listarPartidos` ahora filtra los partidos sin cupos disponibles. Siguen siendo visibles para el organizador en "Mis Partidos" (para poder eliminarlos o gestionarlos), solo se ocultan del listado público.
- Se agregaron 4 tests nuevos (`testUnirseConMensajeQuedaGuardado`, `testEliminarPartidoPorOrganizador`, `testNoOrganizadorNoPuedeEliminar`, `testNoSePuedeEliminarPartidoAMenosDeUnaHora`, `testListarPartidosExcluyeLlenos`) — los 13 tests automatizados pasan.
- Revisión de responsive: se encontró y corrigió la tarjeta de solicitud pendiente (`.applicant-card`), que en pantallas angostas apretaba el mensaje del jugador entre el avatar y los botones Aceptar/Rechazar en una sola fila. Ahora se apila en columna por debajo de 480px.
